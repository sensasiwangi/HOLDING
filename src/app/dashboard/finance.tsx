...
import {
  ...
  Building2, Calendar, ShoppingBag, Store, Globe, FileSpreadsheet,
} from "lucide-react";
...
export default function FinancePanel() {
  ...
  // Parse dashboard data: rows 4-9 (Dalam sheet, rows 0-indexed after header)
  // Dashboard format: [Divisi, Pemasukan, Pengeluaran, Laba/Rugi, Setoran 30%, Sisa Setelah Setoran]
  const rows = data.dashboard || [];
  // Find header row and divisi rows
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (row[0] === "Divisi" || row[0] === "TOTAL") {
      ...
      else tableRows.push(row); // TOTAL row
    } else if (row[0] && DIVISIS.some((d) => d.key === row[0])) {
      tableRows.push(row);
    }
  }

  // Compute summary from tableRows (might be empty if data not yet filled)
  ...
  // Ensure we include TOTAL row if present
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            color: totalLabaRugi >= 0 ? "text-green-600" : "text-red-500",
            bg: totalLabaRugi >= 0 ? "bg-green-50" : "bg-red-50",
          },
          {
            label: "Setoran 30%",
            value: totalSetoran,
          ...
          <div
            key={c.label}
            className="border border-[var(--line)] rounded-xl bg-white p-4"
          >
            ...
            <div
              className={`text-lg font-extrabold mt-1 ${c.color}`}
            >
              {fmt(c.value)}
            </div>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs text-[var(--muted)]">
              Sisa Setelah Setoran 30%
            </div>
            <div
              className={`text-2xl font-extrabold mt-1 ${
                sisaSetoran >= 0 ? "text-green-600" : "text-red-500"
              }`}
            >
              {fmt(sisaSetoran)}
            </div>
        ...
        {tableRows.filter((r) => r[0] !== "TOTAL").length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-[var(--muted)] mt-1">
              Isi data di Google Sheets untuk melihat rekap di sini
            </p>
          </div>
        ) : (
              ...
              <tbody>
                ...
                {totalRow && (
                  <tr className="bg-[var(--soft)] font-bold">
                    ...
                    <td className="py-2.5 px-3 text-right text-green-700">
                      {fmt(safeNum(totalRow[1]))}
                    </td>
                    <td className="py-2.5 px-3 text-right text-red-600">
                      {fmt(safeNum(totalRow[2]))}
                    </td>
                    <td
                      className={`py-2.5 px-3 text-right ${
                        safeNum(totalRow[3]) >= 0
                          ? "text-green-700"
                          : "text-red-600"
                      }`}
                    >
                      {fmt(safeNum(totalRow[3]))}
                    </td>
                    <td className="py-2.5 px-3 text-right text-blue-700">
                      {fmt(safeNum(totalRow[4]))}
                    </td>
                    <td
                      className={`py-2.5 px-3 text-right ${
                        safeNum(totalRow[5]) >= 0
                          ? "text-green-700"
                          : "text-red-600"
                      }`}
                    >
                      {fmt(safeNum(totalRow[5]))}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
          {DIVISIS.map((d) => {
            ...
            return (
              <div
                key={d.key}
                className="border border-[var(--line)] rounded-lg p-3 bg-[var(--soft)]"
              >
      ...
      <div className="border border-[var(--line)] rounded-xl bg-white p-5">
        ...
        {data.rekapSetoran && data.rekapSetoran.length > 1 ? (
          ...
          <div className="text-center py-6">
            <p className="text-sm text-[var(--muted)]">
              Belum ada data setoran bulanan
            </p>
          </div>
        )}
      </div>

      {/* Jumlah Transaksi Banner */}
      <div className="border border-[var(--line)] rounded-xl bg-white p-5">
        <div className="text-xs text-[var(--muted)] mb-3">
          Kolom Jumlah (Rp) yang terbaca dari Sheets:
        </div>
        <div className="grid grid-cols-3 gap-3 text-center">
          <div className="p-3 bg-green-50 rounded-lg">
            <div className="text-xs text-green-700">Pemasukan</div>
          ...
          <div className="p-3 bg-blue-50 rounded-lg">
            <div className="text-xs text-blue-700">Setoran 30%</div>
      ...
      <div className="text-center text-xs text-[var(--muted)] py-4">
        <p>
          Data otomatis tersinkronisasi dari Google Sheets. Angka laba/rugi = pemasukan − pengeluaran tiap divisi. Setoran = 30% × pemasukan tiap divisi.
        </p>
        <p className="mt-1">
          Terakhir diperbarui:{" "}
          {data.fetchedAt
